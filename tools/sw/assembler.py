...
def source_lines(source, tree, sources, stack=()):
    ...
    for number, line in enumerate(text.splitlines(), 1):
        ...
        if re.match(r'INCLUDE\b', statement, re.I):
            ...
            if len(parts) != 2:
                raise AssemblyError('SYNTAX', 'INCLUDE requires a quoted path', location)
            try:
                name = string(parts[1])
...
def assemble(source, tree, prelude, assets=None):
    ...
    symbols, equates, section, visibility_spans = {}, {}, None, {}
    ...
    for statement, location in lines:
        try:
            ...
            if mnemonic in ('EXPORT', 'IMPORT'):
                ...
                obj[key].append(name)
                visibility_spans[(key, name)] = location
                continue
            if section is None:
            ...
            error.diagnostic['span'] = location
            if section:
                error.diagnostic.setdefault('section', section['name'])
            raise
    try:
        check_cycles(symbols)
    except AssemblyError as error:
        error.diagnostic['span'] = obj['symbols'][error.diagnostic['symbol']]['span']
        raise
    ...
    for name in obj['exports']:
        if name not in obj['symbols']:
            raise AssemblyError('UNDEFINED_SYMBOL', f'export has no definition: {name}', visibility_spans[('exports', name)], symbol=name)
