...
def interactions(browser, base):
    ...
    try:
        ...
        expect(page.locator('#path')).to_have_text('README.md')
        for category in ('Src', 'Agents/Skills', 'Tools', 'Cfg', 'Presentations', 'Home'):
        ...
        page.locator('#filter').fill('rtl/README.md')
        expect(page.locator('#tree a')).to_have_count(1)
        page.locator('#tree a').click()
        expect(page.locator('#path')).to_have_text('src/rtl/README.md')
        page.locator('#source').click()
        expect(page.locator('#source-dialog')).to_be_visible()
        expect(page.locator('#source-title')).to_have_text('src/rtl/README.md')
        expect(page.locator('#source-lines')).to_contain_text('Synthesizable SystemVerilog')
        ...
        frame.locator('[data-source]').click()
        expect(page.locator('#source-title')).to_have_text('src/rtl/README.md')
        expect(page.locator('#source-dialog')).to_be_visible()
        page.locator('#close-source').click()
        ...
        if page.evaluate('document.fullscreenEnabled'):
            ...
            frame.locator('[data-source]').click()
            expect(page.locator('#source-dialog')).to_be_visible()
            assert page.evaluate('document.fullscreenElement.contains(document.querySelector("#source-dialog"))')
            page.locator('#close-source').click()
            page.locator('#fullscreen').click()
