...
def interactions(browser, base):
    ...
    try:
        ...
        stats_frame = page.frame_locator('#document iframe')
        expect(stats_frame.get_by_role('heading', name='Repository statistics', exact=True)).to_be_visible()
        expect(stats_frame.locator('#source table')).to_be_visible()
        page.screenshot(path=str(OUTPUT / 'statistics-desktop.png'), full_page=False)
        ...
        expect(page.locator('#document')).to_contain_text('Independent adapter boundaries')
