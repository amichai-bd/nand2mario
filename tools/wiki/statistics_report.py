...
main{max-width:1240px;margin:auto;padding:40px 30px 70px}a{color:var(--accent)}a:focus-visible{outline:3px solid var(--accent);outline-offset:4px}
...
.grid{display:grid;grid-template-columns:1fr 1fr;gap:20px}.scroll{overflow-x:auto;border:1px solid var(--border);border-radius:var(--radius);margin:18px 0}
...
@media(max-width:700px){main{padding:24px 16px 40px}.cards{grid-template-columns:repeat(2,minmax(0,1fr))}.grid{grid-template-columns:1fr}th,td{padding:9px 12px}.card{padding:14px}.chart{padding:14px}}
@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}}@media print{body{background:white;color:black}main{padding:0}.cards,.grid{break-inside:avoid}.scroll{overflow:visible}a{color:inherit}}
'''
...
def chart(title, rows, field, color):
    ...
    return f'<div class="chart"><h3>{text(title)}</h3><svg xmlns="http://www.w3.org/2000/svg" width="840" height="270" viewBox="0 0 840 270" role="img" aria-label="{text(title)}; values also in the timeline table"><title>{text(title)}</title>{"".join(shapes)}</svg><small>UTC dates; last 14 active dates at most. Full values below.</small></div>'
...
<p class="muted">Manual snapshot · source <code>{text(source['revision'])}</code><br>GitHub: {text(github['observed_at']) if github else 'not collected'}</p></header>
<nav aria-label="Report sections"><a href="#source">Source</a><a href="#delivery">Delivery</a><a href="#timeline">Timeline</a><a href="#files">File types</a><a href="#method">Method</a></nav>
