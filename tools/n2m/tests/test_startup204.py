...
import tempfile
import unittest
from src.dv.ppu.startup204 import build

ROOT=Path(__file__).resolve().parents[3]


class Startup204Tests(unittest.TestCase):
    def test_original_images_and_independent_boundary(self):
        with tempfile.TemporaryDirectory(dir=ROOT/'workdir') as folder:
            for case,count,halt,wanted in [('read',142,620,255),('write',145,644,129)]:
                destination=Path(folder)/case
                image=build(ROOT,destination,case)
                report=json.loads((destination/'startup204.json').read_text())
                self.assertEqual(len(image),32768)
                self.assertEqual((len(report['events']),report['halt_retirement']),(count,halt))
                self.assertEqual(report['access_commit']-report['enable_commit'],532)
                self.assertEqual(report['access_retirement'],616)
    ...
    def test_unknown_case_rejected(self):
        with self.assertRaisesRegex(ValueError,'unknown startup204'):
