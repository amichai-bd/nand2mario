...
| Verilator | [Pinned tag, commit and LGPL-3.0-only OR Artistic-2.0 license](../n2m/dependencies.json); built from source by `tools verilator` into the shared host tool cache `<cache>/verilator/v<version>` outside every checkout, which records its own `installation.json`, or into any operator prefix | Supported Linux simulator; no license consulted; no source or binary is committed |
