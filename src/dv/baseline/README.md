...
- **Checks and assertions:** sampled transactions feed the scoreboard; independent local checks reject bad reset and disabled-state changes, and randomized initial values make an unknown output fail by mismatch. End check requires exactly 75 checked transactions and all eight coverage bits.
...
- **Failure artifacts:** seed and exact expected/actual/cycle in sim.log and transactions.csv; baseline.vcd, the harness FST, input/tool fingerprints and raw exit in builder records. Good runs also emit coverage/bins.txt.
...
wrong failure values, missing waves and unexpected mismatches fail. Verilator is the sole backend.
