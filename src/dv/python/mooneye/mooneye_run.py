"""Shared Mooneye run: unmodified pinned selection, real DMG owners and the linked upstream verdict."""
import json
...
async def run(dut, fixture, pc, bank):
    """Load the prepared selection, run to its locked completion and report; the caller names the fixture."""
    ...
    with Path('transactions.jsonl').open('w') as journal, \
        ...
        try:
            ...
            print(f'PASS {fixture} bank={bank} pc={pc:04x} registers=3,5,8,13,21,34', flush=True)
        finally:
