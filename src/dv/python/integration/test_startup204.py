"""Actual CPU VRAM read/write at the primary LCD-on +532-dot boundary."""
import json
...
async def startup_access(dut):
    ...
    contract=json.loads(Path('startup204.json').read_text())
    ...
    with Path('transactions.jsonl').open('w') as trace, Path('retirement.csv').open('w') as records:
        ...
        async def retirement():
            while True:
                ...
                if actual['dot']==616:
                    boundary=next((item for item in accesses if item[0]==612),None)
                    wanted=(612,int(contract['case']=='write'),129 if contract['case']=='write' else 255)
                    assert boundary==wanted,f'STARTUP204_ACCESS expected={wanted} actual={boundary}'
                assert actual==want,f'STARTUP204_RECORD index={i} expected={want} actual={actual}'
                counts['records']+=1
        async def bus():
            while True:
                ...
                if address==0x8000:
                    accesses.append((dot,write,data));log('vram',dot=dot,write=write,data=data)
        async def uart():
            async for encoded in frames(dut):
                decoded=response(encoded[:-1]);log('response',decoded=decoded)
                assert decoded[0]==len(replies),'STARTUP204_RESPONSE_SEQUENCE'
        ...
        while known(dut.dot_count)<contract['halt_retirement']+16:
            ...
            assert current>last,'STARTUP204_PROGRESS'
            assert not known(dut.fault),'STARTUP204_OWNER_FAULT'
            last=current
        for task in tasks:
            if task.done():task.result()
            task.cancel()
        want_access=[(60,1,0),(612,0,255)] if contract['case']=='read' else [(60,1,0),(612,1,129),(636,0,129)]
        want_writes=[(40,0),(80,129)] if contract['case']=='read' else [(40,0),(80,129),(628,0)]
        assert accesses==want_access,f'STARTUP204_BUS {accesses}'
        assert writes==want_writes,f'STARTUP204_WRITES {writes}'
        assert counts['records']==len(expected) and len(replies)==21,'STARTUP204_COMPLETION'
        log('complete',counts=counts,accesses=accesses,writes=writes)
    dut._log.info('PASS startup204 complete CPU VRAM access at enable+532')
