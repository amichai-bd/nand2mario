...
length is 16384, so the engine refuses it; slot 16 is the menu. Slots 0, 1,
2, 4 and 5 declare a tagline and the rest none, so the
[footer](../../../wiki/src/sw/menu/SPEC.md#the-information-footer)'s lower row
is drawn both ways. Ten of the
...
| MBC1 select | Five Downs (the frame that shows slot 5 and the settled frame after it both pixel-exact) then A: the write carries 5; the core boots in `MBC1_ID` with epoch + 1 and result `OK` index 5; the game's bank 2 code returns to the menu (epoch + 2, index still 5) and the menu runs in `LOADER_ID` |
