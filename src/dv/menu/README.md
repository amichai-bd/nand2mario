...
| Delayed catalogue | The menu boots with `sdram_ready` clear: the list settles with the slot numbers alone, no titles and `NOT READY` on the plate. The testbench releases the ready bit after a named frame, the next frame carries the committed bank and the cleared plate, and each frame after it one more title row, every one pixel-exact, ending on the whole list with bank 34 and both window bits set. `menu-delayed` releases it at once; `menu-delayed-worst` holds it seven frames longer, which is what puts the sixteen-letter title of slot 7 in the same VBlank as the nudge phase change |
...
| `menu-delayed` | `delayed` | `PASS menu-delayed checks=21 frames=18 selects=0 commands=6` |
| `menu-delayed-worst` | `delayed-worst` | `PASS menu-delayed-worst checks=28 frames=25 selects=0 commands=6` |
...
label of its own, because its alignment is structurally seven frames longer
than the ordinary per-simulation target permits and it runs as a declared wall
allowance, so every aggregate stays
