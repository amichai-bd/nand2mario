...
| Tile range | Every background map cell the menu writes names a tile in its 82-tile bank; a write outside it fails with `MENU_TILE_RANGE`. This bounds a row drawn with the wrong inverse offset wherever it runs, including the delayed catalogue path no fixture reaches yet |
