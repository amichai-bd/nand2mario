...
module tb_uart_endpoint;
    ...
    logic peek_read, peek_valid, peek_ready;
    ...
    n2m_memory_stores u_memory (.oam_request('0), .oam_response(),
        ...
        .core_paused(paused),.oam_sequence_active(1'b0),.peek_ready(peek_ready),.peek_read(peek_read),.peek_select(peek_select),.peek_offset(peek_offset),
        .peek_rdata(peek_rdata),.peek_valid(peek_valid)
    );
