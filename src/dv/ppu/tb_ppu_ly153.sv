...
module tb_ppu_ly153;
    ...
    task automatic check_read(input logic [7:0] line_value, input logic flag_value,
                              input logic irq_value, input logic event_value = 0);
        io_address = 16'hff44;
        #1;
        ...
        cases = cases + 1;
    endtask
    task automatic initialize_case(input logic [7:0] compare);
        @(negedge clk_sys);
        core_reset = 1;
        pause_request = 0;
        pending_write = 0;
        ...
        elapsed = 0;
    endtask
    task automatic fixed_compare(input logic [7:0] compare);
        initialize_case(compare);
        advance_to(69760);
        check_read(152, compare == 152, compare == 152);
        advance_to(69761);
        check_read(152, 0, compare == 152);
        advance_to(69763);
        if ($test$plusargs("ly_corrupt")) force dut.readable_ly = 8'd0;
        check_read(153, 0, compare == 152);
        advance_to(69766);
        check_read(153, 0, compare == 152);
        advance_to(69767);
        check_read(0, compare == 153, compare == 153, compare == 153);
        advance_to(69768);
        check_read(0, compare == 153, compare == 153);
        advance_to(69769);
        check_read(0, 0, compare == 153);
        advance_to(69772);
        check_read(0, 0, compare == 153);
        advance_to(69773);
        if ($test$plusargs("irq_corrupt")) force dut.stat_rise = 1'b0;
        check_read(0, compare == 0, compare == 0, compare == 0);
    endtask
    initial begin
        ...
        check_read(0, 0, 0);
        if (cases != 45) $fatal(1, "PPU_LY153_COUNT actual=%0d", cases);
        $display("PASS PPU LY153 public reads cases=45 legal LYC invalid valid pause reset");
        $finish;
    end
    initial begin
        #100000000;
