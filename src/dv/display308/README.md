...
The ordinary LYC projection is separate from the exceptional LY153 table:
LY increments at B-5, phase0 captures the comparison at B-3, and the following
active dot registers the condition at B-2. The post-NBA passive sampler records
that condition with the updated completed-dot count. IF consumes it on a later
