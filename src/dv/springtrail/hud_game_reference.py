...
from composition_reference import scene as old_scene
from hud_reference import image,hud_tiles,column

def scene(game):
    return old_scene(game)[:64]+bytes(96)
...
class Check:
    def __init__(self,short=False):
        ...
        self.images=[bytes(23040),image(Game())]
    ...
    def pixel(self,value):
        ...
        assert self.lcd is not None and frame<2,'HUD_EXTRA_PIXEL'
        y,x=divmod(index,160)
        want=(2<<21)|(x<<13)|(y<<5)|(self.images[frame][index]<<3)|(int(index==0)<<2)|int(frame!=0)
        assert value&((1<<53)-1)==want,f'HUD_PIXEL frame={frame} index={index}'
        dot=value>>53
        assert self.lcd+frame*PERIOD+y*456<=dot<self.lcd+frame*PERIOD+(y+1)*456,'HUD_ROW'
    ...
    def line(self,text):
        ...
        if kind=='END':
            assert int(raw)==self.lines and not self.partial,'HUD_END'
        ...
        if kind=='I':
            ...
            assert not self.short and not self.inputs and epoch==2 and buttons==129,'HUD_INPUT'
            assert self.lcd is not None and self.lcd+60000<=dot<=self.lcd+62000,'HUD_INPUT_WINDOW'
            self.inputs.append(dot);return
        if kind=='R':
            from test_integration import decode_record
            row=decode_record(value)
            assert row['seq']==self.records and row['epoch']==2 and row['dot']>self.last_record,'HUD_RETIRE'
            assert not row['halt_bug'],'HUD_HALT_BUG'
            if row['kind']==1:
                assert row['pc_after'] in (0x40,0x48),'HUD_IRQ_VECTOR'
                self.irq.append((row['dot'],row['pc_after']))
            if self.triggers and self.triggers[-1]+8<=row['dot']<=self.triggers[-1]+644:
                assert row['kind']==0 and 0xff80<=row['pc_before']<=0xfffe and row['ime']==0,'HUD_DMA_IRQ'
            self.records+=1;self.last_record=row['dot'];return
        assert kind=='W','HUD_TRACE_KIND'
        ...
        if address==0xff40:
            if data==0:
                assert self.lcd is None,'HUD_LCD_OFF'
            elif self.lcd is None:
                assert data==0x91 and 100000<dot<160000,'HUD_STARTUP_BOUND'
                self.lcd=dot
            else:
                assert data in (0x91,0x93),'HUD_OBJECT_MODE'
        if 0xc100<=address<0xc1a0:
            index=len(self.ready)
            assert index<2 and address==0xc100+len(self.partial),'HUD_SHADOW_ORDER'
            self.partial.append(data)
            if len(self.partial)==160:
                assert bytes(self.partial)==scene(self.states[index]),'HUD_SHADOW'
                assert bytes(self.memory[a] for a in ADDRESSES)==state_bytes(self.states[index],0 if index==0 else 129),'HUD_STATE'
                if index==0:assert self.lcd is None,'HUD_INITIAL_READY'
                else:assert self.lcd+PERIOD<=dot<self.lcd+PERIOD+49280,'HUD_VISIBLE_READY'
                self.ready.append(dot);self.partial=[]
        if address==0xff46:
            n=len(self.triggers)
            assert n<3 and data==0xc1,'HUD_DMA_TRIGGER'
            if n==0:assert len(self.ready)==1 and self.lcd is None,'HUD_INIT_DMA'
            else:
        ...
        if address==0xc019 and self.lcd is not None:
            assert 65664<=(dot-self.lcd)%PERIOD<70224 and data==129,'HUD_JOYP_SAMPLE'
            self.samples.append(dot)
        if address==0xc050 and data==1:
            assert self.lcd is not None,'HUD_TOKEN_STARTUP'
            n=len(self.tokens)
            assert self.lcd+n*PERIOD+65662<=dot<=self.lcd+n*PERIOD+65852,'HUD_TOKEN_TIME'
            self.tokens.append(dot)
        if 0xc220<=address<=0xc226:
            index=len(self.hud)
            assert index<2 and address==0xc220+len(self.hud_partial),'HUD_CACHE_ORDER'
            self.hud_partial.append(data)
            if len(self.hud_partial)==7:
                assert bytes(self.hud_partial)==hud_tiles(self.states[index]),'HUD_CACHE_DATA'
                self.hud.append(dot);self.hud_partial=[]
        if 0xc200<=address<0xc220:
            assert len(self.cache)<32 and address==0xc200+len(self.cache),'HUD_COLUMN_CACHE_ORDER'
            self.cache.append(data)
            assert data==(column(0)+column(1))[len(self.cache)-1],'HUD_COLUMN_CACHE_DATA'
            assert self.lcd+PERIOD<=dot<self.lcd+PERIOD+49280,'HUD_COLUMN_CACHE_READY'
        if self.lcd is not None and dot>self.lcd:
            ...
            if address in (0xff40,0xff43) and phase<65664:
                ...
                assert 65664<=phase<PERIOD,'HUD_DISPLAY_WINDOW'
                assert phase<65664+4480,'HUD_PUBLICATION_BOUND'
    ...
    def bus(self,raw):
        ...
        if self.bus_trigger is not None and self.bus_trigger+8<=dot<=self.bus_trigger+644:
            assert 0xff80<=address<=0xfffe,'HUD_DMA_BUS'
            self.bus_count+=1
    ...
    def finish(self,pause,tile_bytes):
        ...
        assert self.bus_count>0 and self.records>100,'HUD_MISSING_PROGRESS'
        publications=1 if self.short else 3
        assert len(self.triggers)==publications and len(self.dma)==160*publications,'HUD_DMA_COUNT'
        for i,value in enumerate(self.dma):
            publication,offset=divmod(i,160)
            want=scene(self.states[max(0,publication-1)])[offset]
            assert (value>>18,(value>>8)&255,value&255)==(self.triggers[publication]+8+4*offset,offset,want),'HUD_DMA_BYTE'
        if self.short:
            assert 160<=self.pixels<640 and len(self.ready)==1 and self.input_dot is None,'HUD_SHORT_END'
        else:
            assert self.pixels==46080 and len(self.ready)==2 and len(self.hud)==2,'HUD_FRAME_COUNT'
            assert len(self.cache)==32 and len(self.tokens)==2 and len(self.samples)==2,'HUD_UPDATE_COUNT'
            assert [vector for _,vector in self.irq]==[0x48,0x40,0x48,0x40],'HUD_IRQ_ORDER'
            assert [row[0] for row in self.split]==[0,0,1,1],'HUD_SPLIT_COUNT'
            expected_sources=[(self.lcd+6838,2),(self.lcd+65662,1),(self.lcd+PERIOD+6838,2),(self.lcd+PERIOD+65662,1)]
            assert self.sources==expected_sources,'HUD_IRQ_SOURCES'
            for (dot,vector),(request,source) in zip(self.irq,self.sources):
                assert 20<=dot-request<=52,'HUD_IRQ_LATENCY'
            ...
            assert columns==[(0x9c40+x+y*32,v) for x in (0,1) for y,v in enumerate(column(x))],'HUD_RESTORE_WRITES'
            assert self.lcd+60000<=self.input_dot<=self.lcd+62000,'HUD_INPUT_WINDOW'
            assert self.inputs==[self.input_dot],'HUD_APPLIED_INPUT'
