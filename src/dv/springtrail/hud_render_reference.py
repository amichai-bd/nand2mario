...
from hud_game_reference import Check as GameCheck, PERIOD, scene
...
class Check(GameCheck):
    def __init__(self, short=False):
        ...
        self.game=Game(mode=1,player=Player(x=120*16,y=12*16,camera=97))
        self.images=[bytes(23040),image(self.game)]
    ...
    def line(self,text):
        ...
        if address==0xff40:
            if data==0:assert self.lcd is None,'HUD_LCD_OFF'
                ...
                assert data==0x99 and 0<dot<150000,'HUD_STARTUP_BOUND'
                self.lcd=dot
            else:assert data in (0x99,0x9b),'HUD_OBJECT_MODE'
        if 0xc100<=address<0xc1a0:
            assert not self.ready and address==0xc100+len(self.partial),'HUD_SHADOW_ORDER'
            self.partial.append(data)
            if len(self.partial)==160:
                assert bytes(self.partial)==scene(self.game) and self.lcd is None,'HUD_SHADOW'
                self.ready.append(dot);self.partial=[]
        if address==0xff46:
            n=len(self.triggers)
            assert n<2 and data==0xc1 and len(self.ready)==1,'HUD_DMA_TRIGGER'
    ...
    def finish(self,pause,tile_bytes):
        ...
        for i,value in enumerate(self.dma):
            ...
            assert (value>>18,(value>>8)&255,value&255)==(self.triggers[pub]+8+4*offset,offset,scene(self.game)[offset]),'HUD_DMA_BYTE'
        ...
        return dict(pixels=self.pixels,records=self.records,lcd=self.lcd,ready=self.ready,dma_bytes=len(self.dma),pause=pause,irq=self.irq,split=self.split,terminal=self.terminal,bus_count=self.bus_count)
