"""Continuous actual-system pause/Select-restart proof with passive public trace."""
import json
...
async def run(dut, short=False):
    ...
    with Path('transactions.jsonl').open('w') as journal:
        ...
        try:
            ...
            @bridge
            def press(buttons):return client.control('INPUT',buttons)
            with Path('springtrail.trace').open() as trace:
                ...
                while True:
                    ...
                    await Timer(10 if not short and len(check.triggers)==check.count else 50,unit='us')
                    ...
                    prior=dot;consume()
                    assert dot<WATCHDOG,'PAUSE_WATCHDOG'
                    if check.lcd is not None:
                        if short and check.pixels>=160:break
                        if not short and len(check.triggers)==check.count and dot>check.triggers[-1]+644:break
                    n=check.next_input(dot)
                    if n is not None:
                        low,high=check.input_window(n)
                        assert dot<=high,'PAUSE_INPUT_LATE'
                        refresh_clock(client);reply=await press(SCRIPT[n])
                        # Generated INPUT reply contains the applied dot, not acknowledgement time.
                        check.input_dots.append(reply['dot'])
                        log('input_ack',index=n,applied_dot=reply['dot'],ack_dot=known(dut.dot_count),buttons=SCRIPT[n])
                        assert low<=reply['dot']<=high,'PAUSE_INPUT_WINDOW'
                ...
                healthy()
                assert (known(dut.dot_count),known(dut.record_sample),len(check.dma),known(dut.paused))==(pause,*before,1),'PAUSE_PAUSED_HOLD'
                assert not any(known(s) for s in (dut.fault,dut.reset_sys,dut.core_reset)),'PAUSE_PAUSED_FAULT'
    ...
    dut._log.info('PASS PAUSE game complete frames=%d', check.pixels//23040)
