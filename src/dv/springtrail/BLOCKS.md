...
histories into 59-byte operand snapshots: the contact/power fixture's 48 bytes
plus the eleven block bytes at C078..C082. The same program builder, checker
...
block, pause holding every block byte and `InitGame` clearing them from a dirty
state.
...
Startup LCD commit moves from 146500 to 167792 dots: 20732 for `InitBlockArt`
...
returns and the outer `CALL`), 528 for initializing 36 rather than 14 gameplay
bytes, and 32 for the scene's new effect test. This is an instruction-derived
