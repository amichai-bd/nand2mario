...
def build(root, destination, variant='motion'):
    ...
    try:
        ...
        if variant == 'power':
            ...
            secondary = dict(pose=12, facing=32, x=60, y=32, address=SECONDARY)
        validate(root)
        ...
        lines += ['DEC B', 'JR NZ,TileBlock', 'LD HL,$9800', 'LD B,64',
                  ...
                  'JR NZ,ClearHUD', 'CALL InitHUD', 'CALL InitMotionArt', 'CALL InitBlockArt', 'CALL InitProgressArt', 'CALL InitEntityArt', 'LD A,$E4',
                  ...
                  'ASSET "Courier"', 'SeedValues:', 'DB '+','.join(str(v) for v in state_bytes(game))]
        ...
        # Shared scene clears all160 bytes; the secondary courier replaces only
        # unused tail after the fixed current entity slots (capacity checked by model).
