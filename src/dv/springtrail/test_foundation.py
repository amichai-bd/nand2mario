...
from reference import Check, START_WINDOW, END


@cocotb.test(timeout_time=90, timeout_unit='ms')
async def foundation(dut):
    ...
    with Path('transactions.jsonl').open('w') as journal:
        ...
        @bridge
        def start():return client.control('INPUT',128)
        try:
    ...
    dut._log.info('PASS SPRINGTRAIL foundation title start world')
