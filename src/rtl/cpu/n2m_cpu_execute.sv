...
module n2m_cpu_execute (
    ...
    always_comb begin
        ...
        alu_rhs = read_byte(request.registers, request.opcode[2:0]);
        ...
        if (request.opcode[7:6] == 0 && (request.opcode[2:0] == 4 || request.opcode[2:0] == 5)) begin
            alu_operation = request.opcode[0] ? ALU_DEC : ALU_INC;
            alu_lhs = request.opcode[5:3] == 6 ? request.temporary[7:0] : read_byte(request.registers, request.opcode[5:3]);
        end
        case (request.opcode)
        ...
        if (request.cb_bank) begin
            alu_lhs = request.opcode[2:0] == 6 ? request.temporary[7:0] : read_byte(request.registers, request.opcode[2:0]);
            case (request.opcode[7:6])
                0: alu_operation = ALU_RLC + {2'b0, request.opcode[5:3]};
    ...
    always_comb begin
        ...
        result.plan.access_kind = ACCESS_IDLE;
        result.finish = 0;
        result.prefix = 0;
        ...
        pair_value = read_pair(request.registers, request.opcode[5:4], 0);
        ...
        taken = condition_true(request.opcode[4:3], request.registers.f);
        return_step = request.step;

        if (request.cb_bank) begin
            if (request.opcode[2:0] == 6 && request.step == 0) begin
                result.plan.access_kind = ACCESS_DATA;
                ...
                if (request.opcode[2:0] != 6) begin
                    result.registers_after = write_byte(request.registers, request.opcode[2:0], alu_value);
                    result.registers_after.f = alu_flags;
            ...
            if (request.opcode == 8'h76) begin
                ...
                if (request.opcode[5:3] == 6) begin
                    result.plan.write_enable = 1;
                    result.plan.write_data = read_byte(request.registers, request.opcode[2:0]);
                end else result.temporary_after[7:0] = request.data;
            end else begin
                result.finish = 1;
                if (request.opcode[5:3] != 6)
                    result.registers_after = write_byte(request.registers, request.opcode[5:3], request.opcode[2:0] == 6 ? request.temporary[7:0] : read_byte(request.registers, request.opcode[2:0]));
            end
            ...
            if (request.opcode[5:3] == 6 && request.step == 0) begin
                ...
                result.finish = 1;
                if (request.opcode[5:3] != 6) result.registers_after = write_byte(request.registers, request.opcode[5:3], alu_value);
                result.registers_after.f = alu_flags;
            end
        end else if (request.opcode[7:6] == 0 && request.opcode[2:0] == 6) begin
            if (request.step == 0) begin
                result.plan.access_kind = ACCESS_OPERAND;
                ...
                result.finish = 1;
                if (request.opcode[5:3] != 6) result.registers_after = write_byte(request.registers, request.opcode[5:3], request.temporary[7:0]);
            end
            ...
            if (request.step < 2) begin
                ...
                result.finish = 1;
                result.registers_after = write_pair(request.registers, request.opcode[5:4], 0, request.temporary);
            end
        end else if (request.opcode[7:6] == 0 && (request.opcode[3:0] == 3 || request.opcode[3:0] == 11)) begin
            if (request.step == 0)
                result.registers_after = write_pair(request.registers, request.opcode[5:4], 0, request.opcode[3] ? pair_value - 16'd1 : pair_value + 16'd1);
            ...
            if (request.step == 0) begin
                result.plan.access_kind = ACCESS_DATA;
                result.plan.address = request.opcode[5] ? {request.registers.h, request.registers.l} : read_pair(request.registers, {1'b0, request.opcode[4]}, 0);
            ...
            if (request.step < 2) begin
                ...
                result.plan.access_kind = ACCESS_STACK;
            ...
            pair_value = read_pair(request.registers, request.opcode[5:4], 1);
            ...
            if (request.step < 2) begin
                ...
                result.finish = 1;
                result.registers_after = write_pair(request.registers, request.opcode[5:4], 1, request.temporary);
            end
        ...
        if (result.plan.access_kind == ACCESS_OPERAND) begin
        ...
        if (!request.cb_bank) begin
            if (request.opcode[7:6] == 0 && (request.opcode[3:0] == 3 || request.opcode[3:0] == 11) && request.step == 0) begin
                result.address_effect.valid = 1;
                result.address_effect.address = read_pair(request.registers, request.opcode[5:4], 0);
            end
            ...
            if ((request.opcode == 8'hc9 || request.opcode == 8'hd9 || (request.opcode & 8'he7) == 8'hc0) &&
                    result.plan.access_kind == ACCESS_STACK && return_step == 0) begin
                result.address_effect.valid = 1;
                result.address_effect.address = request.registers.sp;
            end
        end
        ...
        if (result.finish) begin
            result.plan.access_kind = ACCESS_OPCODE;
