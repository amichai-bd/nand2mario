...
module n2m_cpu_alu (
    ...
    always_comb begin
        ...
        case (operation)
            ALU_ADD, ALU_ADC: begin // ADD, ADC
                carry_in = (operation == ALU_ADC) && flags_in[4];
            ...
            ALU_SUB, ALU_SBC, ALU_CP: begin // SUB, SBC, CP
                carry_in = (operation == ALU_SBC) && flags_in[4];
                wide_result = {1'b0, lhs} - {1'b0, rhs} - {8'b0, carry_in};
                low_result = {1'b0, lhs[3:0]} - {1'b0, rhs[3:0]} - {4'b0, carry_in};
                value = wide_result[7:0];
                flags_out = {value == 8'b0, 1'b1, low_result[4], wide_result[8], 4'b0};
                if (operation == ALU_CP) value = lhs;
            end
            ALU_AND, ALU_XOR, ALU_OR: begin // AND, XOR, OR
                case (operation)
                    ALU_AND: value = lhs & rhs;
                    ALU_XOR: value = lhs ^ rhs;
                    default: value = lhs | rhs;
                endcase
                flags_out = {value == 8'b0, 1'b0, operation == ALU_AND, 1'b0, 4'b0};
            end
            ALU_INC: begin // INC preserves C.
                value = lhs + 8'd1;
                flags_out = {value == 8'b0, 1'b0, lhs[3:0] == 4'hf, flags_in[4], 4'b0};
            end
            ALU_DEC: begin // DEC preserves C.
                value = lhs - 8'd1;
                flags_out = {value == 8'b0, 1'b1, lhs[3:0] == 4'h0, flags_in[4], 4'b0};
            end
            ALU_DAA: begin // DAA uses N to distinguish add/subtract correction.
            ...
            ALU_CPL: begin // CPL preserves Z/C.
                value = ~lhs;
                flags_out = {flags_in[7], 2'b11, flags_in[4], 4'b0};
            end
            ALU_SCF: flags_out = {flags_in[7], 2'b00, 1'b1, 4'b0}; // SCF
            ALU_CCF: flags_out = {flags_in[7], 2'b00, ~flags_in[4], 4'b0}; // CCF
            ALU_RLCA, ALU_RLC: begin // RLCA, RLC
                value = {lhs[6:0], lhs[7]};
                flags_out = {(operation == ALU_RLC) && value == 0, 2'b00, lhs[7], 4'b0};
            end
            ALU_RRCA, ALU_RRC: begin // RRCA, RRC
                value = {lhs[0], lhs[7:1]};
                flags_out = {(operation == ALU_RRC) && value == 0, 2'b00, lhs[0], 4'b0};
            end
            ALU_RLA, ALU_RL: begin // RLA, RL
                value = {lhs[6:0], flags_in[4]};
                flags_out = {(operation == ALU_RL) && value == 0, 2'b00, lhs[7], 4'b0};
            end
            ALU_RRA, ALU_RR: begin // RRA, RR
                value = {flags_in[4], lhs[7:1]};
                flags_out = {(operation == ALU_RR) && value == 0, 2'b00, lhs[0], 4'b0};
            end
            ALU_SLA: begin // SLA
                value = {lhs[6:0], 1'b0};
            ...
            ALU_BIT: flags_out = {~lhs[bit_index], 1'b0, 1'b1, flags_in[4], 4'b0};
            ALU_RES: value = lhs & ~(8'b1 << bit_index);
            ALU_SET: value = lhs | (8'b1 << bit_index);
            default: begin
                value = lhs;
