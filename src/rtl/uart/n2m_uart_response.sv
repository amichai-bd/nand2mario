...
module n2m_uart_response (
    ...
    n2m_interfaces_pkg::packet_header_t header, new_header;
    logic [n2m_uart_pkg::UART_ADDRESS_BITS-1:0] index, index_next;
    logic [15:0] crc, crc_next;
    assign new_header = {payload_bytes, status, command, sequence_token, n2m_interfaces_pkg::WIRE_RESPONSE, n2m_interfaces_pkg::WIRE_VERSION};
    ...
    `N2M_ASSERT(UART_RESPONSE_LENGTH, clk_sys, reset_sys,
        start |-> payload_bytes <= n2m_interfaces_pkg::WIRE_MAX_PAYLOAD && (status == n2m_interfaces_pkg::STATUS_OK || payload_bytes == 0))
