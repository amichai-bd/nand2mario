...
module n2m_uart_exchange_store (
    ...
    localparam integer BANK_ADDRESS_BITS = n2m_uart_pkg::UART_ADDRESS_BITS;
    genvar bank;
    generate for (bank = 0; bank < 3; bank = bank + 1) begin : banks
        ...
        n2m_intel_ram #(
            ...
            .a_address(write_address[bank*BANK_ADDRESS_BITS +: BANK_ADDRESS_BITS]),
            .a_wdata(write_data[bank*8 +: 8]),
            .a_byte_enable(1'b1), .a_rdata(unused_data), .a_valid(unused_valid),
            .b_read(read_enable[bank]),
            .b_address(read_address[bank*BANK_ADDRESS_BITS +: BANK_ADDRESS_BITS]),
