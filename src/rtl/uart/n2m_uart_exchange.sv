...
module n2m_uart_exchange (
    ...
    `N2M_ASSERT(UART_EXCHANGE_REQUEST_SIZE, clk_sys, reset_sys,
        request_valid |-> 32'(request_bytes) >= n2m_interfaces_pkg::PACKET_HEADER_BYTES + 2 && 32'(request_bytes) <= n2m_uart_pkg::UART_RAW_MAX)
    ...
    `N2M_ASSERT(UART_EXCHANGE_RESPONSE_WRITE, clk_sys, reset_sys,
        response_write |-> command_valid && 32'(response_address) < n2m_uart_pkg::UART_RAW_MAX)
    `N2M_ASSERT(UART_EXCHANGE_COMMAND_DONE, clk_sys, reset_sys,
        command_done |-> command_valid && 32'(response_bytes) >= n2m_interfaces_pkg::PACKET_HEADER_BYTES + 2 && 32'(response_bytes) <= n2m_uart_pkg::UART_RAW_MAX)
