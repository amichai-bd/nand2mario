...
module n2m_uart_packet_rx #(
    ...
    assign encoded_write = state == RECEIVE && rx_valid && !rx_error && rx_data != 0 &&
                           !discard_input && 32'(encoded_count) < n2m_uart_pkg::UART_ENCODED_MAX;
    assign decoded_write = emit_byte && 32'(decoded_count) < n2m_uart_pkg::UART_RAW_MAX;
    ...
    always_comb begin
        ...
        case (state)
            RECEIVE: begin
                if (rx_valid) begin
                    ...
                    if (rx_data == 0) begin
                        ...
                        if (32'(encoded_count) < n2m_uart_pkg::UART_ENCODED_MAX)
                    ...
                    if (32'(idle_count) == TIMEOUT_CYCLES - 1) begin
                        idle_count_next = '0;
                        encoded_count_next = '0;
                        discard_input_next = 1'b0;
            ...
            CHECK: begin
                if (32'(decoded_count) >= n2m_interfaces_pkg::PACKET_HEADER_BYTES + 2 && crc == tail &&
        ...
        if (emit_byte) begin
            if (32'(decoded_count) >= n2m_uart_pkg::UART_RAW_MAX) begin
                state_next = RECEIVE;
                encoded_count_next = '0;
            end else begin
                if (32'(decoded_count) < n2m_interfaces_pkg::PACKET_HEADER_BYTES)
