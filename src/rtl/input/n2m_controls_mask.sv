...
module n2m_controls_mask #(
    ...
    always_comb begin
        ...
        y_next = y_q;
        if (pair_valid) begin
            x_next = classify_axis(pair_x, x_q, X_MIN, X_CENTER, X_MAX);
            y_next = classify_axis(pair_y, y_q, Y_MIN, Y_CENTER, Y_MAX);
        end
        if (!fresh) begin
            x_next = AXIS_CENTER;
            y_next = AXIS_CENTER;
    ...
    assign x_negative = x_next == AXIS_NEGATIVE;
    assign x_positive = x_next == AXIS_POSITIVE;
    assign y_negative = y_next == AXIS_NEGATIVE;
    assign y_positive = y_next == AXIS_POSITIVE;
    ...
    `DFF_ARST_VAL(x_q, x_next, clk_sys, reset_sys, AXIS_CENTER)
    `DFF_ARST_VAL(y_q, y_next, clk_sys, reset_sys, AXIS_CENTER)
