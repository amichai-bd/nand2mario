...
// Cyclone V clocking fit proof. Virtual control/observation ports make this a
// fit proof, not a board image; it mirrors the MAX 10 clocking_proof so both
// families prove the same clock contract.
module nano_clocking_proof (
    ...
    n2m_clocking_cyclonev u_clocking (.clk_reference, .clk_sys, .board_reset_n, .clk_pix, .reset_sys, .reset_pix, .ready);
