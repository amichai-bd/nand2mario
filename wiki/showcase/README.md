...
Seven animated SVGs show what using the repository looks like. Three are the
...
each command typed in, the output it really printed, the next command. One is on
this page: Libbet playing on the DE10-Lite, every frame read back off the board.
...
The board loop does not re-read the hardware when it is regenerated. The capture
...
per sampled frame, its sequence, completion dot, applied JOYP mask, the CRC32 of
the packed bytes the board returned, and the encoded payload the SVG embeds:
...
against indexed PNG. On these 28 frames the rect runs cost
3,623,166 bytes against 28,896
for indexed PNG, 125
times smaller, because a Libbet frame fills the screen and the rect form pays
per horizontal run. The rect form exists because the project README sanitizes
the loops it embeds; this one is wiki-only, so the smaller encoding wins. An
inline `data:` URI fetches nothing, so the loop stays as self-contained as the
rest.
