...
Every pixel of every other loop is drawn on the host. The board loops are the
exception, and they are the only place the wiki shows a frame the FPGA
produced.
...
each archive as one animated tile per game.
...
![Nine games running on the DE10-Lite](games-gallery.svg)

The landing page gallery: one tile per game, each a three-frame flipbook of
captures the DE10-Lite returned over UART. It reads nine committed archives and
adds none of its own, so it needs no capture session: Springtrail's three frames
come from `springtrail-board.json`, Stackdrop's from `stackdrop-board.json`,
Libbet's from `libbet-board.json`, and each pinned homebrew game's are the three
its own archive holds.
...
page. The Springtrail and Stackdrop tiles say the repository builds them
instead.
...
rather than nine because the site fits a figure to the text column: nine
separate loops would be nine column-wide blocks to scroll past.

Nine tiles fill the 3x3 grid, so the note that used to sit in its ninth cell is
now a two-column band under the grid, and the footer stays anchored to the
bottom below it. At 1024x1266 the gallery fills the text column at either
breakpoint, and at the narrow one the whole grid lands inside a phone screen. The tiles stay
...
## Stackdrop on the board

![Stackdrop captured on the DE10-Lite](stackdrop-board.svg)

Three native 160x144 frames from one
[`board_play.py`](../../src/dv/stackdrop/board_play.py) session on 2026-09-14:
the title, the well filling with the L still in flight, and the hard drop that
completes the bottom row, clears it and takes the score to 0100. The session
loaded the built image with `host load --package`, which matched its SHA-256 and
read all 32768 bytes back, and every retained frame was decoded by
[`screen.py`](../../src/dv/stackdrop/screen.py) before it was kept.

This is boot-and-play evidence, not a correctness proof. Stackdrop's rules are
proved against the independent reference model in
[`src/dv/stackdrop/`](../../src/dv/stackdrop/README.md), in simulation; nothing
in this capture re-checks them. The button pills stay dark because the driver
captures each frame after the press is released, so no frame carries a mask.
