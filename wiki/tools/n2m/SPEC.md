...
Both also check every SystemVerilog target's
[preload fixture inputs](#preload-fixtures-under-verilator), so a target that
would refuse to run fails here instead of at simulation time: an input the
builder reads but the target does not declare fails as
`registry <target>: preload_inputs omit fixture inputs: <paths>`.
