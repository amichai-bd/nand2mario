...
scaled image and writes two checked-in includes: the colour table, and the
border map and tile ROM arrays the scanout reads. They are constants, so the
fitter builds the border art from logic; the
[border selection](../../src/rtl/vga/MAS_vga.md#border-selection) states why an
initialized block memory is unavailable on this device configuration.
...
hold the drift check, the encoding facts, the absence of any memory template,
and the agreement of the generated art with the
