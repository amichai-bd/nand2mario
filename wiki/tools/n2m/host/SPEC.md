...
and `PROFILE_*` constants, so the host tool and the RTL share one source. The
same functions drive the `library-peer` Verilator target through the
simulation peer, so the wire bytes are those a board receives.
