...
| `host load --external <name>` | Same transmission and verification from a pinned external image fetched at run time; `LOAD_BEGIN` carries the generated ID of the pin's `profile` (absent means `dmg-direct-v1`; `dmg-mbc1-v1` pins are 64 KiB `MBC1_ID` sessions), and the whole image is read back. Exactly one of `--package` or `--external` is accepted. |
...
| `host sdram-write --address <integer> --data <hex digits>` | Write 1 through 15 whole 16-byte lines (32 hex digits each, byte 0 first) at a line-aligned SDRAM device address in one generated `SDRAM_WRITE`; the line count is the data length; alignment, whole lines, the 15-line limit and device bounds are checked before the port opens. |
...
| `host sdram-test [--start <integer> --length <bytes> \| --full \| --boundary] [--seed <n>]` | Write a seeded address-dependent pattern over the range fifteen lines per `SDRAM_WRITE`, read it back fifteen lines per `SDRAM_READ` and compare; every mismatching line is listed by device address with expected and actual bytes (first 64 in detail, all counted). Default range is one 32 KiB slot at 0; `--full` covers the 64 MiB device: 279,621 commands each way, about 4 hours at 115200 baud. `--boundary` instead writes and reads back the [storage contract's boundary set](../../../src/rtl/storage/MAS_sdram.md#verification): first and last line of slots 0, 15 and 16, both catalogue edges, first and last line of a row and one line in each bank including the device end, naming each mismatching line. The command fails when any line mismatches. |
| `host library load <result.json>... [--menu <result.json>]` | Fill the [sixteen-slot library](../../../src/rtl/storage/MAS_sdram.md#address-space-layout): each immutable software attempt is validated by the packager before the port opens (loose ROMs, foreign profiles and more than sixteen images are refused), placed from slot 0 in order, a 32 KiB image in one slot and a 64 KiB `dmg-mbc1-v1` image in two adjacent slots (its entry at the first, the second empty; together at most sixteen slots, a 64 KiB image never starting in slot 15), written fifteen lines per `SDRAM_WRITE`, the `--menu` image to index 16, then the 17-entry catalogue (valid, profile, 24-bit length, CRC32, header title, reserved) and its tagline table at `0x88000`. The packager ([`package.py`](../../../../tools/n2m/host/package.py)) admits each attempt at the exact image length of its profile. Every write completes before the readback: each slot is read by `SDRAM_READ`, compared byte for byte and by CRC32 against its catalogue entry, and the catalogue bytes are compared. The library table is printed (JSON: `result.slots`, `result.catalogue`); any mismatch names the slot and its title in `error` and fails the command with the table retained. The `profile` byte is the generated ID of the profile the package record names: `dmg-direct-v1` is `DIRECT_ID`; the [loader-profile menu image](../../../src/sw/menu/SPEC.md) is `LOADER_ID`. The [contract](../../../src/rtl/cartridge/MAS_loader_profile.md#boot-source) accepts either ID at index 16, so a direct-profile `--menu` image is catalogued as `DIRECT_ID` and the loader-profile menu as `LOADER_ID`. Every slot, catalogue and profile number is the generated `LIBRARY_*` or `PROFILE_*` constant. Titles are printed with control bytes replaced by `?`. Does not load the menu into the ROM store or run anything. |
| `host library status` | Read the catalogue as stored and print its 17 rows (index, name, valid, profile, length in bytes, CRC32, title, tagline); the raw bytes land in `catalogue.bin`. Also reads `LIBRARY_STATUS` into `result.library_status`: the `$A000` flags by generated name, the `$A002` result code with its generated name, `$A003` and the window bank. |
| `host library return [--wait]` | Return to the menu from the host: one whitelisted `WRITE_HOST(LIBRARY_CONTROL)` of `LIBRARY_CONTROL_RETURN`, which the [loader profile](../../../src/rtl/cartridge/MAS_loader_profile.md#host-interaction) treats exactly like the KEY1 hold. The endpoint `STATE` is read first and a `LOADING` endpoint is refused by name before anything is sent; a `BAD_STATE` reply is reported by name. After the write, `LIBRARY_STATUS` is read once (with `--wait`, until `copy_busy` and `key1_pending` clear, within `RETURN_STATUS_READS` reads; the command fails if the swap has not settled by then) and `STATE`, `PROFILE` and `IMAGE_VALID` are read. `result` carries the decoded status (flags, result code name, `$A003`, bank), `endpoint` (state by name, profile, image validity), `before`, `status_reads` and `settled`. The result code is reported, not judged: a healthy return reads `OK` with the loader profile live. The console keeps its host state through the swap ([select register, step 6](../../../src/rtl/cartridge/MAS_loader_profile.md#select-register)): a host-paused console reads `PAUSED` until `host run`, a running console reads `RUNNING`, so a `PAUSED` result is not a failed return. |
...
`Client.sdram_write(address, lines)` (1 through 15 whole lines in one
command), `Client.sdram_write_region(address, data, notify)` (a line multiple
of bytes in fifteen-line commands), `Client.sdram_read(address, lines)` and
...
define; `interface_codec.sdram_write` and `sdram_line_address` reject an
empty or ragged line set, more than fifteen lines, or a misaligned or
out-of-device range by name before anything is sent, and the endpoint answers
...
line commands: `build_catalogue` packs the generated `catalogue_entry`
records through `interface_codec.pack_record` and `parse_catalogue` reads them
back, together with the
[tagline table](../../../src/rtl/storage/MAS_sdram.md#address-space-layout)
behind them: `image_entry` takes an authored tagline, `check_tagline` refuses
anything the menu font cannot draw, and a slot that declares none packs an
all-zero record, so a catalogue holding no tagline is byte for byte the one
written before the table existed. The whole 1 KiB region is written and
...
`host.library.return_to_menu(client, wait=)` implements the return verb over
`Client.write_host` and `READ_HOST`. The same functions drive the
`library-peer` Verilator target through the simulation peer, so the wire bytes
are those a board receives.
