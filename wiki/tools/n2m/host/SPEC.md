...
| `host sdram-test [--start <integer> --length <bytes> \| --full] [--seed <n>]` | Write a seeded address-dependent pattern over the range one line per `SDRAM_WRITE`, read it back fifteen lines per `SDRAM_READ` and compare; every mismatching line is listed by device address with expected and actual bytes (first 64 in detail, all counted). Default range is one 32 KiB slot at 0; `--full` covers the 64 MiB device and takes hours at 115200 baud. The command fails when any line mismatches. |
