...
location rule and the creation step; it builds the environment when absent and
reuses it otherwise, writing the ready marker last so an interrupted install is
...
skips the unit by name only when that build cannot complete. No command installs
the package into the caller's interpreter.
