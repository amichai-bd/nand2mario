...
the producer or emulated time. The index is sequential 0 through 23039. Abort
...
| snapshot_valid / snapshot_metadata | Published validity and generated snapshot_t, including size 5760. Metadata changes only on successful completion or global reset. |
| frame_read / frame_address[12:0] | Byte request from the published host snapshot. The caller requires snapshot_valid and address less than 5760. |
...
one done/ok=0 response, including when reset remains asserted for several edges.
...
two host banks. Four shades pack into each byte with the earliest pixel in bits 1:0.
