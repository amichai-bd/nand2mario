...
| `library-peer` | The product host Client drives `host library load`, `host library status` and `host library return` through the simulation peer against the endpoint, controller and model: two `sw build` images plus a menu image are written to their slots and the catalogue, read back and verified by CRC32 and bytes; the catalogue read by the status verb equals the one written; the return verb's whitelisted `LIBRARY_CONTROL` write raises exactly one `library_return` pulse at the endpoint (no loader is present, so `LIBRARY_STATUS` reads as the driven zero word) ([`tb_library_peer`](../../../../src/dv/storage/tb_library_peer.sv), [`library_peer.py`](../../../../src/dv/storage/library_peer.py)) |
...
each way), about 4 hours in all; the full-device board result is the
[sweep record](../../board-bring-up.md#full-sdram-sweep) on the board bring-up
page.
