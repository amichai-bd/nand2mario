...
|---|---|---|
| Direct ROM | Memory, generated exact profile size | Host endpoint validates LOAD_BEGIN/presence/CRC and controls loading; memory accepts bounded byte offsets and supports complete readback. CPU ROM writes never load bytes. The [loader profile](../cartridge/MAS_loader_profile.md#copy-engine-and-rom-store-port-ownership) is the second writer of the host port, through its port arbiter, and observes the resolved CPU ROM write commits as its bank and select registers and, in the direct and MBC1 profiles, as the [game exit register](../cartridge/MAS_loader_profile.md#game-exit-register): a write of `$10` to `$6000`-`$7FFF` returns to the menu; every other direct-profile ROM write stays ignored. The store holds `PROFILE_STORE_BYTES`; the [MBC1 profile](../cartridge/MAS_mbc1_profile.md#store-and-loader-interplay) owner translates switched-window reads into its upper half and is the identity for the 32 KiB profiles. |
...
|---|---|---|
| Resolved access | Read/write, store selector, sixteen-bit store offset (the fifteen-bit CPU offset after the [MBC1 owner](../cartridge/MAS_mbc1_profile.md#address-map-in-the-mbc1-profile)'s translation, zero-extended elsewhere) and byte data. Offset must fit the selected generated region. | Registered byte and valid after a read edge. ROM writes have no effect. |
