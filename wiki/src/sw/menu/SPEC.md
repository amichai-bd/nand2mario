...
| `menu-scroll` | The [scroll ramp](#the-scroll-ramp): slot 15 is deposited into the menu's `Cursor` byte at a frame's first pixel, which the menu treats as a move, and the next four frames carry SCY 146, 148, 150 and 152, the header sliding off, `LAST SLOT` sliding in above the plate and the pointer riding its row over the plate, the footer staged on the first; Up on the scrolled frame ramps back over four frames to the settled slot 14. Every frame is pixel-exact and every ramp frame's VBlank is measured. The deposit is the testbench's: fourteen Downs at two frames each would cost more than the per-simulation target |
| `menu-select-last` | Slot 14 deposited the same way stages and settles like a move; Down crosses the boundary by a joypad edge, the same four ramp frames follow, and A on the scrolled frame commits 15 to the select register: `LAST SLOT` boots in `DIRECT_ID` with epoch + 1 and result `OK` index 15, so every slot 0..15 is reachable and selectable |
