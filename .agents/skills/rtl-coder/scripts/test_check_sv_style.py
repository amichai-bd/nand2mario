import unittest
from check_sv_style import checked, legacy_declarations, violations


class DeclarationStyleTests(unittest.TestCase):
    def test_only_systemverilog_is_checked(self):
        for name in ["src/rtl/cpu/n2m_cpu.sv", "src/rtl/common/macros.svh"]:
            self.assertTrue(checked(name))
        for name in ["src/dv/cpu/cpu_lint.vlt", "src/dv/builder/targets.json", "tools/build.py"]:
